  ...
  else
  {
    ...
    switch (GattProcId)
    {
      ...
      case PROC_GATT_PROPERTIES_ENABLE_ALL:
      {
        ...
        {
          if(((a_ClientContext[index].P2PNotificationDescHdl) & CHAR_PROP_NOTIFY) == CHAR_PROP_NOTIFY)
          {
            charPropVal = 0x0001;
          }
          if(((a_ClientContext[index].P2PNotificationDescHdl) & CHAR_PROP_INDICATE) == CHAR_PROP_INDICATE)
          {
            charPropVal = 0x0002;
          }
